...
def train():
  ...
  optimizer = optim.SGD(model.parameters(), lr=learning_rate, weight_decay=1e-1)
  ...
  while n_iterations < max_steps:
    ...
    acc = accuracy(out.data.numpy(), y.data.numpy())
    ...
    if (n_iterations+1) % eval_freq  == 0:
      ...
      acc_test = accuracy(u_out.data.numpy(), v.data.numpy())
      test_accuracy.append(acc_test)
      print("Test accuracy at iteration {}:".format(n_iterations+1), acc_test)
    n_iterations += 1

  with open("pytorch_mlp_results_modified.txt", "w")  as f:
    f.write("PyTorch MLP Modified (Init and weight decay): Train acc, train loss and test acc\n")
